...
use super::records::*;
use super::serializer_config::{SerializerConfig, SerializerMode};
use super::string_field_patcher::StringPatchInfo;
...
pub struct EnhancedSerializer {
    ...
    string_changes: Vec<StringPatchInfo>,
}
...
pub fn serialize_enhanced_with_config(
    ...
    rec.validate_graph()?;
    EnhancedSerializer::new(config).serialize(rec)
}
...
impl EnhancedSerializer {
    ...
    fn serialize(mut self, rec: &DeserializedRecord) -> Result<Vec<u8>, String> {
        ...
        if self.config.log_record_summary && !self.string_changes.is_empty() {
            eprintln!("\n✓ Serialization completed successfully");
            eprintln!("  String changes tracked: {}", self.string_changes.len());
            for change in &self.string_changes {
                eprintln!("  {}", change.diagnostic_message());
            }
        }
    ...
    fn validate_record_size(
        ...
        match self.config.mode {
            SerializerMode::FlexibleStrings => {
                // 允许字符串记录（type 6）、类记录（types 1, 4, 5）以及
                // BinaryArray（type 7）的大小变化。
                // String records 是自描述的（带有 7-bit 编码长度前缀），
                // BinaryArray 的长度字段明确编码在二进制格式中，
                // 所以可以安全地改变大小（例如增删背包物品）。
                if matches!(metadata.original_record_type, 1 | 4 | 5 | 6 | 7) {
                    if self.config.verbose_diagnostics && metadata.original_record_type == 6 {
                        if let Some(Record::String(value)) = recs.records.get(&id) {
                            eprintln!(
                                "✓ String record {} allowed size change: {} → {} bytes (value: '{}')",
                                id, metadata.original_byte_length, serialized_size, value
                            );
                        }
                    }
                    return Ok(());
            ...
            SerializerMode::Strict => {
                // Strict 模式下不进行任何宽恕
            }
            SerializerMode::Adaptive => {
                // 未来可能的自适应逻辑
                if matches!(metadata.original_record_type, 1 | 4 | 5 | 6 | 7) {
                    return Ok(());
                }
